        ...
        for s = sn
            ...
            switch experiment
                ...
                case 'sc2'
                    runTrue    = runB;
                    runTruestr = run{2};
            end % in experiment 1, the order of the runs is different for two subjects

            for r = runLst
                P = getrow(A,A.runNum == runTrue(r)); %% contains the data (start time, end timem etc.) for behavioral runs without the data for instructions
                N = cell(1, numTRs - numDummys);   %% preallocating the variable that will contain the paths pointing to the image files
                for it = 1:(numTRs-numDummys)
                    N{it} = fullfile(baseDir, 'sc1', imDir, subj_name{s}, sprintf('%srun_%s.nii,%d', prefix, runTruestr{r}, it)); %% all the imaging data is uner sc1/imaging_data
                end % scans (volume images)
                J.sess(r).scans = N; % path to scans
                
                itt = 1;
                ic0 = 2; %% task condition index (not including the first index which is the instruction when reading from Cc)
                for it = 1:length(P.taskNum) % "it" is task index (each task can have multiple conditions)
                    %%% P.taskNum is the number of tasks so it does not
                    %%% include the instruction. But it includes rest!
                    taskName_correct = unique(Cc.taskNames,'rows','stable');
                    ST  = find(strcmp(P.taskName,taskName_correct{it+1})); 
                    
                    fprintf('taskName_correct is %s\n', taskName_correct{it + 1});
                    fprintf('ST is %d\n', ST)
                    
                    for taskType = 1:2 % instruction vs. real task
                        fprintf('taskType is %d\n', taskType);
                        if taskType==1 % instruction
                            fprintf('Doing the instruction\n')
                            task_onset = P.realStartTime(ST)-J.timing.RT*numDummys; %% get the instruction start time for the first task
                            fprintf('task_onset is %d\n', task_onset);
                            condName = Cc.condNames{1};
                            fprintf('the condName should be instruct. Is it? %s\n', condName);
                            duration = Cc.duration(1);
                            fprintf('The duration should be 5 secs. Is it? %d\n', duration)
                            S.inst = 1;
                            S.taskName_after  = P.taskName(ST);
                            if ST>1
                                S.taskName_before = P.taskName(ST-1);
                            ...
                            S.instime        = task_onset;
                            S.instOrder = ST;
                            S.TN    = {condName};
                            S.task  = Cc.taskNum(1);
                            fprintf('taskNum for the instruction should be %d\n', Cc.taskNum(1));
                            S.it    = 0;
                            
                            J.sess(r).cond(itt).name     = condName;
                            J.sess(r).cond(itt).onset    = min(task_onset); % correct start time for numDummys and announcetime included (not for instruct)
                            J.sess(r).cond(itt).duration = duration;  % duration of trials (+ fixation cross) we are modeling
                            J.sess(r).cond(itt).tmod     = 0;
                            J.sess(r).cond(itt).orth     = 0;
                            J.sess(r).cond(itt).pmod     = struct('name', {}, 'param', {}, 'poly', {});
                        else
                            D  = dload(fullfile(baseDir, experiment,behavDir, subj_name{s},sprintf('%s_%s_%s.dat', experiment, subj_name{s}, P.taskName{ST})));
                            R  = getrow(D,D.runNum==runB(r)); % functional runs
                            
                            numCond = length(find(Cc.taskNum == Cc.taskNum(ic0))); %% numCond will be used to calculate ic0 which in turn is used in calculating the onset
                            ...
                            condName = P.taskName{ST};
                            %duration = Cc.duration(ST);
                            duration          = 30;
                            S.inst            = 0;
                            S.instime         = 0;
                            S.taskName_before = 'NaN';
                            S.taskName_after  = 'NaN';
                            S.instOrder       = 0;
                            S.TN              = {condName};
                            S.task            = P.taskNum(ST);
                            S.it              = it;
                            ...
                            J.sess(r).cond(itt).name     = condName;
                            J.sess(r).cond(itt).onset    = min(onset); % correct start time for numDummys and announcetime included (not for instruct)
                            J.sess(r).cond(itt).duration = duration;  % duration of trials (+ fixation cross) we are modeling
                            J.sess(r).cond(itt).tmod     = 0;
                            J.sess(r).cond(itt).orth     = 0;
                            J.sess(r).cond(itt).pmod     = struct('name', {}, 'param', {}, 'poly', {});
...
%                         J.sess(r).cond(it).name     = condName
%                         J.sess(r).cond(it).onset    = min(task_onset) % correct start time for numDummys and announcetime included (not for instruct)
%                         J.sess(r).cond(it).duration = duration  % duration of trials (+ fixation cross) we are modeling
...
%                     instruct_onset = P.realStartTime(it)-J.timing.RT*numDummys; %% get the instruction start time for the first task
                    ...
                    J.sess(r).cond(itt).name     = Cc.condNames{1};
                    J.sess(r).cond(itt).onset    = instruct_onset(1); % correct start time for numDummys and announcetime included (not for instruct)
                    J.sess(r).cond(itt).duration = Cc.duration(1);  % duration of trials (+ fixation cross) we are modeling
                    J.sess(r).cond(itt).tmod     = 0;
                    J.sess(r).cond(itt).orth     = 0;
                    J.sess(r).cond(itt).pmod     = struct('name', {}, 'param', {}, 'poly', {});
...
%                     ic       = ic + 1;
                    ...
                    for cond=1:numCond 
                        ...
                        onset = [P.realStartTime(ST)+R.startTimeReal(tt)+announceTime-(J.timing.RT*numDummys)];
                        my_onset = [my_onset; onset];
                        trial_dur = Cc.duration(ic0);
...
%                         ic  = ic + 1;
                    ...
                    itt  = itt + 1;
                    J.sess(r).cond(itt).duration = trial_dur;
                    J.sess(r).cond(itt).name     = P.taskName{ST};
                    J.sess(r).cond(itt).onset    = my_onset;
%                     J.sess(r).cond(itt).duration = Cc.duration(itt);
                    J.sess(r).cond(itt).tmod     = 0;
                    J.sess(r).cond(itt).orth     = 0;
                    J.sess(r).cond(itt).pmod     = struct('name', {}, 'param', {}, 'poly', {});
                    S.SN    = s;
...
%                     S.cond  = Cc.condNum(ic0);
    ...
    case 'GLM:mdtb:design_glm10'% Same as GLM7, except in GLM10, rest is also modeled. GLM7 was using the text file that did not have a "rest" condition and hence rest was not modeled in GLM7. Here, I'm running the same thing with the "new" text file that has rest.
        % So the main difference between glm7 and glm10 will be that glm10
        % has rest as a condition/task.
        %%% This case will calculate the design matrix with the instruction
        %%% period for each task separated and coming before the task.
...
%                     instruct_onset = P.realStartTime(it)-J.timing.RT*numDummys; %% get the instruction start time for the first task
             ...
             %       for cond=1:numCond 
                        ...
                        % determine trialType (ugly -- but no other way)
                        % this is only needed when more than one condition
                        tt = 1;
%                         if isfield(R,'trialType')
%                             tt = (R.trialType==Cc.trialType(ic0));
%                         else
...
%                         end
                        ...
                        % loop through trial-types (ex. congruent or incongruent)
                        J.sess(r).cond(ic).name     = Cc.taskNames{ic0};
                        J.sess(r).cond(ic).onset    = onset; % correct start time for numDummys and announcetime included (not for instruct)
                        J.sess(r).cond(ic).duration = Cc.duration(ic0);  % duration of trials (+ fixation cross) we are modeling
                        J.sess(r).cond(ic).tmod     = 0;
                        J.sess(r).cond(ic).orth     = 0;
...
%                         runTrue = runB;
            ...
            for r = runLst
                ...
                ic0 = 2; %% task condition index (not including the first index which is the instruction when reading from Cc)
                ic  = 1; %% I call this the universal index :)
                taskUn = unique(Cc.taskNames);
                taskUn = taskUn(~strcmp(taskUn,'Instruct'));
                for it = 1:length(taskUn) % "it" is task index (each task can have multiple conditions)
                    %%% P.taskNum is the number of tasks so it does not
                    %%% include the instruction. But it includes rest!
                    % Instructions first
                    ST  = find(strcmp(P.taskName,taskUn{it}));
                    instruct_onset = P.realStartTime(ST)-J.timing.RT*numDummys; %% get the instruction start time for the first task                   
                    J.sess(r).cond(ic).name     = Cc.condNames{1};
                    J.sess(r).cond(ic).onset    = instruct_onset(1); % correct start time for numDummys and announcetime included (not for instruct)
                    J.sess(r).cond(ic).duration = Cc.duration(1);  % duration of trials (+ fixation cross) we are modeling
                   ...
                   % S.cond  = 0;
                    ...
                    ic  = ic + 1;
                    D  = dload(fullfile(baseDir, experiment,behavDir, subj_name{s},sprintf('%s_%s_%s.dat', experiment, subj_name{s}, taskUn{it})));
                    R  = getrow(D,D.runNum==runB(r)); % functional runs
                    ST = find(strcmp(P.taskName,taskUn{it}));
                   ...
                   % tt = 1;
                    ...
                    onset = [P.realStartTime(ST)+R.startTimeReal(1)+announceTime-(J.timing.RT*numDummys)];
                    
                    % loop through trial-types (ex. congruent or incongruent)
                    J.sess(r).cond(ic).name     = taskUn{it};
                    J.sess(r).cond(ic).onset    = onset; % correct start time for numDummys and announcetime included (not for instruct)
                    J.sess(r).cond(ic).duration = unique(Cc.duration(strcmp(Cc.taskNames, taskUn{it})));  % duration of trials (+ fixation cross) we are modeling
                    J.sess(r).cond(ic).tmod     = 0;
                    J.sess(r).cond(ic).orth     = 0;
                    J.sess(r).cond(ic).pmod     = struct('name', {}, 'param', {}, 'poly', {});
                    ...
                    S.task  = unique(Cc.taskNum(strcmp(Cc.taskNames,taskUn{it})));
                    %   S.cond  = Cc.condNum(ic0);
                    S.TN    = {taskUn{it}};
                    S.sess  = sess(r);
                    S.deriv = 0;
...
%             taskName = Ccc.taskNames;
%         end
%% used in creating the dataframe, using non-normalized betas only
% for s = sn
%             % load in SPM_info.mat
%             T   = load(fullfile(glmDir, subj_name{s}, 'SPM_info.mat'));
%             ts1 = load(fullfile(tsDir, subj_name{s}, sprintf('Ts_%s_%s.mat', taskName, roi1)));
%             ts2 = load(fullfile(tsDir, subj_name{s}, sprintf('Ts_%s_%s.mat', taskName, roi2)));
%             b1  = ts1.ts.beta;
%             b2  = ts2.ts.beta;
%             
%             %%% there are length(runLst) intercepts. Omit them:
%             b1 = b1(1:end - length(runLst), :);
%             b2 = b2(1:end - length(runLst), :);
%             
%             %%% discarding the derivative regressors
%             dind = T.deriv == 0; %% use this variable to get the beta values for regressors other than the derivatives 
%             T    = getrow(T, dind == 1);
%             
%             b1 = b1(dind, :);
%             b2 = b2(dind, :);
...
%             df_tmp.SN = s;
%             for i = 0:length(unique(T.cond))- 1
%                 ind    = T.cond == i;
%                 b1_tmp = b1(ind, :);
%                 b2_tmp = b2(ind, :);
%                 
%                 %%% run will be the third dimension, data for each run is
%                 %%% on a "plane"
%                 b1_tmp = reshape(b1_tmp', [size(b1_tmp, 2), size(b1_tmp, 1)/length(runLst), length(runLst)]);
%                 b2_tmp = reshape(b2_tmp', [size(b2_tmp, 2), size(b2_tmp, 1)/length(runLst), length(runLst)]);
%                 
%                 a = getrow(T, T.cond == i);
%                 
%                 % create separate condition names for each instruction.
...
%                 if i == 0 % the indicator for the instructions is 0. Separating 16 instructions!
%                     for k = 1:nTask % there are 16 instructions
%                         df_tmp.b1{1, 1} = b1_tmp(:, k, :);
%                         df_tmp.b2{1, 1} = b2_tmp(:, k, :);
%                         
%                         %%% calculate mean across runs
%                         mbeta1_tmp = mean(b1_tmp(:, k, :), 3);
%                         mbeta2_tmp = mean(b2_tmp(:, k, :), 3);
%                         
%                         %%% calculate mean across the whole structure (ROI)
%                         mmbeta1_tmp    = mean(mbeta1_tmp);
%                         mmbeta2_tmp    = mean(mbeta2_tmp);
%                         df_tmp.mmbeta1 = mmbeta1_tmp;
%                         df_tmp.mmbeta2 = mmbeta2_tmp;
% 
%                         %%% calculate the std across runs
%                         sbeta1_tmp = std(b1_tmp(:, k, :), 0, 3);
%                         sbeta2_tmp = std(b2_tmp(:, k, :), 0, 3);
%                         
%                         %%% calculate std across the whole structure (ROI)
%                         ssbeta1_tmp    = std(mbeta1_tmp);
%                         ssbeta2_tmp    = std(mbeta2_tmp);
%                         df_tmp.ssbeta1 = ssbeta1_tmp;
%                         df_tmp.ssbeta2 = ssbeta2_tmp;
% 
%                         df_tmp.mbeta1{1, 1} = mbeta1_tmp; 
%                         df_tmp.mbeta2{1, 1} = mbeta2_tmp;
% 
%                         df_tmp.sbeta1{1, 1} = sbeta1_tmp; 
%                         df_tmp.sbeta2{1, 1} = sbeta2_tmp;
%                 
%                         df_tmp.cName = cellstr(sprintf('%s_%2.2d', char(unique(a.TN)), k));
%                         df_tmp.cond  = i;
%                         df_tmp.task  = 0; %% instructions are task 0
%                         df = addstruct(df, df_tmp); 
%                     end
%                 else % do the whole thing for all the other task conditions
%                     df_tmp.b1{1, 1} = b1_tmp;
%                     df_tmp.b2{1, 1} = b2_tmp;
%                     
%                     mbeta1_tmp = mean(b1_tmp, 3);
%                     mbeta2_tmp = mean(b2_tmp, 3);
%                     
%                     mmbeta1_tmp = mean(mbeta1_tmp);
%                     mmbeta2_tmp = mean(mbeta2_tmp);
% 
...
%                     df_tmp.mmbeta2 = mmbeta2_tmp;
% 
%                     sbeta1_tmp = std(b1_tmp, 0, 3);
%                     sbeta2_tmp = std(b2_tmp, 0, 3);
% 
%                     df_tmp.mbeta1{1, 1} = mbeta1_tmp; 
%                     df_tmp.mbeta2{1, 1} = mbeta2_tmp;
% 
...
%                     df_tmp.sbeta2{1, 1} = sbeta2_tmp;
%                 
%                     df_tmp.cName = unique(a.TN);
%                     df_tmp.cond  = i;
%                     df_tmp.task  = unique(a.task);
%                     df = addstruct(df, df_tmp);
%                 end
%             end
%         end
...
%                 B{r}.SN       = subj_name{s};
%                 B{r}.mbetasNW = mean(beta_hat, 2);
%                 B{r}.mbetasUW = mean(betasUW, 2);
%% to go from suit to individual
%         % maps each atlas of the suit into individual space
%         % Example: sc1_sc2_mdtb('ROI:mdtb:suit2native', 'sn', [3])
%         sn         = 1:31;
%         parcelType = 'Buckner_17';                    %% set it to Buckner_7 or Buckner_17
%         parcelDir  = fullfile(suitToolDir, 'atlas'); %% directory where the nifti image is stored
%         experiment = 1;
%         
%         vararginoptions(varargin, {'sn', 'parcelType', 'parcelDir', 'experiment'});
%         
%         experiment = sprintf('sc%d', experiment);
%         
%         parcelFile = fullfile(parcelDir, sprintf('%sNetworks.nii', parcelType));
%         
%         spm('defaults','fmri');
%         spm_jobman('initcfg');
% 
%         for s = sn
%             % atlas -> native space
%             subjAnatInv = fullfile(baseDir, experiment, suitDir, 'anatomicals', subj_name{s}, 'maskbrainSUITGrey.nii'); % inverse transformation
%             outDir = fullfile(baseDir, experiment, suitDir, 'anatomicals', subj_name{s});
%             dircheck(outDir);
%             
%             % read the networks file
%             atlasV = spm_vol(parcelFile);
%             V      = spm_read_vols(atlasV);
%             
%             nativeFile = atlasV;
%             
%             % copy original file
%             nativeFile.fname = fullfile(outDir, sprintf('i%sNetworks.nii', parcelType));
...
%             % space version
%             delete(fullfile(outDir,sprintf('i%sNetworks.nii', parcelType)));
%             fprintf('******************** %s for %s native space done ********************\n\n',parcelType,subj_name{s}); 
%         end % sn
%% Defining buckner regions in the cerebellum. This case was then added to
% another case which defines the ROIs both for the cerebellum and the
% cortex
%     case 'ROI:mdtb:define_suit'
%         % Defines cerebellar ROIs using Buckner 7 and 17 networks
%         % Example: sc1_sc2_mdtb('ROI:mdtb:define_suit', 'sn', [3])
%         
%         sn         = 1:31;
%         parcelType = 'Buckner_7';
%         experiment = 1;
...
%         parcelDir  = fullfile(suitToolDir, 'atlasesSUIT'); %% directory where the nifti image is stored
%          
%         vararginoptions(varargin, {'sn', 'type', 'experiment', 'ppmethod', 'glm'});
%         
%         experiment = sprintf('sc%d', experiment);
%         
%         switch ppmethod
...
%         
%         for s = sn
% %             mask = fullfile(baseDir, experiment, suitDir, 'anatomicals', subj_name{s}, 'maskbrainSUITGrey.nii');
%             mask = fullfile(glmDir, subj_name{s}, 'mask.nii'); %% the functional map created in GLM
%             
%             CV          = spm_vol(fullfile(parcelDir, sprintf('%sNetworks.nii', parcelType)));
%             CX          = spm_read_vols(CV);
%             nparcel     = length(unique(CX)) - 1; % 0 will also be discarded
...
%             R = region_calcregions(R);
%             dircheck(fullfile(baseDir, experiment,regDir,'data',subj_name{s}));
%             save(fullfile(baseDir, experiment, regDir,'data',subj_name{s},sprintf('regions_%s.mat',parcelType)),'R');
%             fprintf('\n******************** %s Rois were defined for %s ********************\n\n', parcelType, subj_name{s});
%         end % sn
%% The old case I used to create the beta dataframe
%     case 'EA:mdtb:beta_dataframe'
%         % creates an "Evoked Activation" dataframe
%         % Example: sc1_sc2_mdtb('EA:mdtb:beta_dataframe', 'sn', [3, 4, 7])
...
%             % load in SPM_info.mat
%             T   = load(fullfile(glmDir, subj_name{s}, 'SPM_info.mat'));
%             % load in the beta values 
%             B1   = load(fullfile(betaDir, subj_name{s}, sprintf('beta_regions_%s.mat', roi1)));
%             B2   = load(fullfile(betaDir, subj_name{s}, sprintf('beta_regions_%s.mat', roi2)));
%             B1   = B1.B;
%             B2   = B2.B;
%             
%             % calculating the mean across all ROIs of the structure.
%             %%% if the number of ROIs present in the structure is 1, then
%             %%% the ROI is probably the whole structure
%             if size(B1.(sprintf('mbetas%s', normmode)), 1) ~= 1
%                 mB1 = (mean((B1.(sprintf('mbetas%s', normmode))), 1))';
%             else 
%                 mB1 = (B1.(sprintf('mbetas%s', normmode)))';
%             end
%             if size(B2.(sprintf('mbetas%s', normmode)), 1) ~= 1
%                 mB2 = (mean((B2.(sprintf('mbetas%s', normmode))), 1))';
%             else
%                 mB2 = (B2.(sprintf('mbetas%s', normmode)))';
%             end
%             
%             %%% there are length(runLst) intercepts. Omit them:
%             b1 = mB1(1:end - length(runLst), :);
%             b2 = mB2(1:end - length(runLst), :);
%             
%             %%% discarding the derivative regressors
%             dind = T.deriv == 0; %% use this variable to get the beta values for regressors other than the derivatives 
%             T    = getrow(T, dind == 1);
...
%         end
%         fprintf('******************** Beta dataframe created ********************\n\n')
%         save(fullfile(betaDir, sprintf('BetaDf_GLM_%d_regions_%s_regions_%s.mat', glm, roi1, roi2)), 'df', '-v7.3');
%         varargout{1} = df;
%% the case I used to plot the scatter plots
% case 'EA:mdtb:beta_plot'
%         % uses the dataframe structure created in the previous case to plot
%         % the cerebellar activation vs cortical activation.
%         % Example:sc1_sc2_mdtb('EA:mdtb:beta_plot', 'sn', [2, 3, 4, 6, 7, 8, 9, 10, 12, 14, 15, 17, 18, 19, 20, 21, 22, 24, 25, 27, 28, 29, 30, 31])
...
%         normmode   = 'UW';        %% with or without univariate noise normalization
%         
%         vararginoptions(varargin, {'sn', 'ppmethod', 'roi1', 'roi2', 'glm', 'experiment', 'category', 'normmode'});
%         
%         % load in the task information for the subjects
%         D = dload(fullfile(baseDir, 'sc1_sc2_taskConds_GLM.txt'));
%         D = getrow(D, D.StudyNum == experiment);
...
%         load(fullfile(betaDir, sprintf('Beta%sDf_GLM_%d_regions_%s_regions_%s.mat', normmode, glm, roi1, roi2)))
%         
%         % calculate mean and std of beta values across runs for each
%         % subject.
%         %%% Use tapply
%         % fit a linear line to the data, calculate the deviations from the
%         % linear line for each beta (condition)
%         switch ifig
%             case 1
%                 for s = sn
%                     % create the beta vector for each ROI
%                     d_tmp = getrow(df, df.SN == s);
% 
%                     x = d_tmp.mmbeta1;
%                     y = d_tmp.mmbeta2;
% 
%                     figure;
%                     scatterplot(x', y', 'markertype', 'v', 'markercolor', [0.6, 0.2 0], 'markerfill', [0.6, 0.2, 0], 'markersize', 6, 'label', df.cName);
%                     h           = lsline;
%                     h.Color     = 'b';
%                     h.LineWidth = 3;
% 
%                     title(sprintf('beta values for %s vs %s for %s', roi2, roi1, subj_name{s}), 'interpreter', 'none');
%                     xlabel(roi1, 'interpreter', 'none');
%                     ylabel(roi2, 'interpreter', 'none');
% 
%                     si = si + 1;
%                     clear bRoi1_cond bRoi2_cond bRoi1 bRoi2
%                 end
%         end        
%         T = tapply(df, {category}, {'mmbeta1'}, {'mmbeta2'}); % get the mean across subjects
% 
%         figure;
%         xyplot(df.mmbeta1, df.mmbeta2, [], 'split', df.(category), 'errorbars', 'ellipse');
%         hold on
%         scatterplot(T.mmbeta1, T.mmbeta2, 'markertype', 'v',...
%             'markercolor', [0.6, 0.2 0], 'markerfill', [0.6, 0.2, 0],...
%             'markersize', 6, 'label', D.taskNames);
%         h           = lsline;
%         h.Color     = 'b';
%         h.LineWidth = 3;
%         
%         title(sprintf('group average beta values for %s vs %s', roi2, roi1), 'interpreter', 'none');
%         xlabel(roi1, 'interpreter', 'none');
%         ylabel(roi2, 'interpreter', 'none');
%% the case for dataframe        
%     case 'EA:mdtb:beta_dataframe'
%         % creates an "Evoked Activation" dataframe
%         % Example: sc1_sc2_mdtb('EA:mdtb:beta_dataframe', 'sn', [3, 4, 7])
%         sn         = 1:31;       %% subject list
...
%         varargout{1} = df;
%
%% The old case I used for getting the scatter plot of beta values
%     case 'EA:mdtb:beta_plot_scratch'
%         % uses the dataframe structure created in the previous case to plot
%         % the cerebellar activation vs cortical activation.
%         % Example:sc1_sc2_mdtb('EA:mdtb:beta_plot_scratch', 'sn', [2, 3, 4, 6, 7, 8, 9, 10, 12, 14, 15, 17, 18, 19, 20, 21, 22, 24, 25, 27, 28, 29, 30, 31])
%         
%         %%% use xyplot, scatterplot and tapply to reproduce cool figures
%         
% %         sn         = 1:31;        %% list of the subjects
...
%         category   = 'cond';      %% set it to 'cond' or 'task'
%         normmode   = 'UW';        %% with or without univariate noise normalization
%         parcelNum  = 1;           %% parcelNum for which you want to do the plotting
%         xvar       = 'cortex';
%         yvar       = 'cerebellum';
%         
%         plotvar = 'mrmparcel'; %% the variable I want to use for plotting
%         
%         vararginoptions(varargin, {'sn', 'ppmethod', 'roi1', 'roi2', 'glm', 'experiment', 'category', 'normmode', 'parcelNum'});
%         
%         % load in the task information for the subjects
%         D = dload(fullfile(baseDir, 'sc1_sc2_taskConds_GLM.txt'));
%         D = getrow(D, D.StudyNum == experiment);
...
%         
%         % get the dataframe for each structure
%         xdf = getrow(df, strcmp(df.structure, xvar));
%         ydf = getrow(df, strcmp(df.structure, yvar));
%         
%         % get the dataframe for the parcels you want to do the plotting
%         if strcmp(xvar, 'cortex')
%             %%% xdf_L and xdf_R are the data structures for left and right
...
%             %%% all you need to do is to calculate the average between left
%             %%% and right for each subject and each condition
%             xdf_L = getrow(xdf, strcmp(xdf.hemi, 'L'));
%             xdf_R = getrow(xdf, strcmp(xdf.hemi, 'R'));
%             
%             xdf_L_p = getrow(xdf_L, xdf_L.region_num == parcelNum);
%             xdf_R_p = getrow(xdf_R, xdf_R.region_num == parcelNum);
%             
%             T_L = tapply(xdf_L_p, {category}, {plotvar});
%             T_R = tapply(xdf_R_p, {category}, {plotvar});
%         end
%         
%         ydf_p = getrow(ydf, ydf.region_num == parcelNum);
%         
%         T_Y   = tapply(ydf_p, {category}, {plotvar});
%         
%         figure;
%         subplot(121)
%         scatterplot(T_L.(plotvar), T_Y.(plotvar), 'markertype', 'v',...
...
%         h.LineWidth = 3;
%         title(sprintf('vs right hemi for parcel %d', parcelNum))
%% the case for maping group transition contrast into surface without
% smoothing
%     case 'SURF:mdtb:map_con_group_transitions_old'
%         % projects individual contrast map volume files to WorkBench surface
%         % Example: sc1_sc2_mdtb('SURF:mdtb:map_con_group_transition', 'sn', [3])
%     
...
%         con_vs     = 'rest'; %% contrast was calculated against 'rest' or 'average'
%         
%         vararginoptions(varargin,{'sn', 'ppmethod', 'atlas_res', 'experiment', 'glm', 'replaceNaN', 'convs'});
%         
%         experiment = sprintf('sc%d', experiment);
%         
%         switch ppmethod
...
%             end % hemi(h)
%         end % tt (transitions) 
%% An improved case of plotting, but obselete !
%     case 'EA:mdtb:beta_plot'
%         % uses the dataframe structure created in the previous case to plot
%         % the cerebellar activation vs cortical activation.
%         % Example:sc1_sc2_mdtb('EA:mdtb:beta_plot', 'sn', [2, 3, 4, 6, 7, 8, 9, 10, 12, 14, 15, 17, 18, 19, 20, 21, 22, 24, 25, 27, 28, 29, 30, 31])
...
%             xdf_R_p = getrow(xdf_R, xdf_R.region_num == parcelNum);
%             
%             xdf_L_s = getrow(xdf, strcmp(xdf.hemi, 'L'));
%             xdf_R_s = getrow(xdf, strcmp(xdf.hemi, 'R'));
%             
%             T_L = tapply(xdf_L_p, {category}, {plotvar});
%             T_R = tapply(xdf_R_p, {category}, {plotvar});
%             
%             T_L_s = tapply(xdf_L_s, {category}, {plotvar});
%             T_R_s = tapply(xdf_R_s, {category}, {plotvar});
%         end
%         
%         ydf_p = getrow(ydf, ydf.region_num == parcelNum);
%         ydf_s = getrow(ydf, strcmp(ydf.hemi, 'none'));
%         
%         T_Y   = tapply(ydf_p, {category}, {plotvar});
%         T_Y_s = tapply(ydf_s, {category}, {plotvar});
%         
%         figure;
%         subplot(121)
%         %         xyplot(xdf_L_p.(plotvar), ydf_p.(plotvar), [], 'split', df.(category), 'errorbars', 'ellipse');
...
% %         xlabel(xvar)
% %         ylabel(yvar)
%% The old case I used to create flatmaps for the cerebellum. I then replaced this with gifti flatmaps that can be viewed with wb_view
%     case 'SUIT:mdtb:plotgroupflatmap_con_task'
%         % plots and saves the group flatmap for each task contrasts
%         % Example: sc1_sc2_mdtb('SUIT:mdtb:plotgroupflatmap_con_task')
%         
%         ppmethod   = '';      %% with or without stc
%         experiment = 1;       %% enter 1 for sc1 and 2 for sc2
%         type       = 'con';   %% enter the image you want to reslice to suit space
%         glm        = 7;       %% glm number
%         con_vs      = 'rest';  %% is the contrast calculated vs 'rest' or 'average'
%         colorscale = [-3, 3]; %% colorscale for the flatmap
%         
%         vararginoptions(varargin,{'ppmethod', 'experiment', 'glm', 'type', 'colorscale'});
%         
%         % load in task information
%         C        = dload(fullfile(baseDir,'sc1_sc2_taskConds_GLM.txt'));
%         Cc       = getrow(C, C.StudyNum == experiment);
...
%             case ''
%                 glmSuitGroupDir    = fullfile(baseDir, experiment, suitDir, sprintf('glm%d', glm), 'group');
%                 figDir             = fullfile(baseDir, experiment, suitDir, sprintf('glm%d', glm), 'flatmap');
%             case 'stc'
%                 glmSuitGroupDir    = fullfile(baseDir, experiment, suitDir, sprintf('glm%d_stc', glm, 'group'));
%                 figDir             = fullfile(baseDir, experiment, suitDir, sprintf('glm%d_stc', glm), 'flatmap');
%         end
%         dircheck(glmSuitGroupDir);
%         dircheck(figDir);
%         
%         for cc = 1:length(taskNames)
%             fprintf('******************** plotting the group flatmap for %s of %s vs %s ******************** \n\n', type, taskNames{cc}, con_vs);
%             V = spm_vol(fullfile(glmSuitGroupDir, sprintf('group_%s_%s-%s.nii', type, taskNames{cc}, con_vs)));
%             D = suit_map2surf(V,'stats','nanmean');
%             
%             figure; suit_plotflatmap(D, 'cmap', colormap(jet(256)), 'cscale', colorscale);
%             caxis(colorscale);
%             colorbar;
%             
%             title(sprintf('group_%s_%s-%s', type, taskNames{cc}, con_vs), 'Interpreter', 'none');
%             h = gcf;
%             savefig(h, fullfile(figDir, sprintf('group_%s_%s-%s.fig', type, taskNames{cc}, con_vs)));
%             close(h);
%         end % contrast(cc)

%     case 'SUIT:mdtb:plotgroupflatmap_con_cond'
%         % plots and saves the group flatmap for each condition contrasts
%         % Example: sc1_sc2_mdtb('SUIT:mdtb:plotgroupflatmap_con_cond')
%         
%         ppmethod   = '';      %% with or without stc
%         experiment = 1;       %% enter 1 for sc1 and 2 for sc2
%         type       = 'con';   %% enter the image you want to reslice to suit space
...
%             colorbar;
%             
%             title(sprintf('group_%s_%s-%s', type, conNames{cc}, con_vs), 'Interpreter', 'none');
%             h = gcf;
%             savefig(h, fullfile(figDir, sprintf('group_%s_%s-%s.fig', type, conNames{cc}, con_vs)));
%             close(h);
%         end % contrast(cc)

%     case 'SUIT:mdtb:plotgroupflatmap_con_utransitions_names'
%         % plots and saves the group flatmap for each task contrasts
%         % Example: sc1_sc2_mdtb('SUIT:mdtb:plotgroupflatmap_con_utransitions_names')
%         
%         ppmethod   = '';      %% with or without stc
%         experiment = 1;       %% enter 1 for sc1 and 2 for sc2
%         type       = 'con';   %% enter the image you want to reslice to suit space
...
%         switch ppmethod
%             case ''
%                 glmDir             = fullfile(baseDir, experiment, sprintf('GLM_firstlevel_%d', glm)); % where the all_trans.mat is stored
%                 glmSuitGroupDir    = fullfile(baseDir, experiment, suitDir, sprintf('glm%d', glm), 'group');
%                 figDir             = fullfile(baseDir, experiment, suitDir, sprintf('glm%d', glm), 'flatmap');
%             case 'stc'
%                 glmSuitGroupDir    = fullfile(baseDir, experiment, suitDir, sprintf('glm%d_stc', glm, 'group'));
%                 figDir             = fullfile(baseDir, experiment, suitDir, sprintf('glm%d_stc', glm), 'flatmap');
%                 glmDir             = fullfile(baseDir, experiment, sprintf('GLM_firstlevel_%d_stc', glm)); % where the all_trans.mat is stored
%         end % switch ppmethod
%         dircheck(glmSuitGroupDir);
%         dircheck(figDir);
%         
%         % load in all_trans.mat containing the task names for unique
%         % transitions
%         load(fullfile(glmDir, 'all_trans_task_names.mat'));
%         
%         for tt = 1:size(all_trans, 1)
%             fprintf('******************** plotting the group flatmap for %s of %s vs %s ******************** \n\n', type, all_trans{tt, 1}, all_trans{tt, 2}, con_vs);
%             V = spm_vol(fullfile(glmSuitGroupDir, sprintf('group_%s_transition_%s_%s-%s.nii', type, all_trans{tt, 1}, all_trans{tt, 2}, con_vs)));
%             D = suit_map2surf(V,'stats','nanmean');
%             
%             figure; suit_plotflatmap(D, 'cmap', colormap(jet(256)), 'cscale', colorscale);
%             caxis(colorscale);
%             colorbar;
%             
%             title(sprintf('group_%s_transition_%s_%s-%s', type, all_trans{tt, 1}, all_trans{tt, 2}, con_vs), 'Interpreter', 'none');
%             h = gcf;
%             savefig(h, fullfile(figDir, sprintf('group_%s_transition_%s_%s-%s.fig', type, all_trans{tt, 1}, all_trans{tt, 2}, con_vs)));
%             close(h);
%         end % contrast(cc)

% case 'SUIT:mdtb:plotgroupflatmap_con_transitions_id'
%         % plots and saves the group flatmap for each task contrasts
%         % Example: sc1_sc2_mdtb('SUIT:mdtb:plotgroupflatmap_con_transitions_id')
%         
%         ppmethod   = '';      %% with or without stc
%         experiment = 1;       %% enter 1 for sc1 and 2 for sc2
%         type       = 'con';   %% enter the image you want to reslice to suit space
...
%         
%         for tt = 1:length(t.instOrder_all)
%             fprintf('******************** plotting the group flatmap for %s of %s vs %s ******************** \n\n', type, t.instOrder_all(tt), con_vs);
%             V = spm_vol(fullfile(glmSuitGroupDir, sprintf('group_%s_transition_%d-%s.nii', type, t.instOrder_all(tt), con_vs)));
%             D = suit_map2surf(V,'stats','nanmean');
%             
%             figure; suit_plotflatmap(D, 'cmap', colormap(jet(256)), 'cscale', colorscale);
%             caxis(colorscale);
%             colorbar;
%             
%             title(sprintf('group_%s_transition_%d-%s', type, t.instOrder_all(tt), con_vs), 'Interpreter', 'none');
%             h = gcf;
% %             savefig(h, fullfile(figDir, sprintf('group_%s_transition_%s-%s.fig', type, t.instOrder_all(tt), con_vs)));
%             close(h);
%         end % contrast(cc)  
%% before I forget
% T = tapply(df, {'cond'}, {'mmbeta1'}, {'mmbeta2'});
% D = dload('sc1_sc2_taskConds_GLM.txt')
% D = getrow(D, D.StudyNum == 1); %% for 'sc1' only
% scatterplot(T.mmbeta1, T.mmbeta2, 'label', D.taskNames);

% the pivottable
% pivottable(T2.taskName_before,T2.taskName_after,T2.inst,'sum')